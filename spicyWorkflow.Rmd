...
cells <- normalizeCells(cells, transformation = NULL, method = c("perc99"), assayIn = "counts")
...
testProp <- colTest(prop[imagesToUse,], condition = clinical[imagesToUse, "Status"])

testProp
boxplot(prop[imagesToUse,rownames(testProp)[1]]~clinical[imagesToUse, "Status"])
...
spicyTest <- spicy(segCells[imagePheno(segCells)$Status%in%c("nonprogressor", "progressor"),], condition = "Status", Rs = c(20, 50, 100))
...
prop <- scFeatures(data, feature_types = c("proportion_raw", "gene_mean_celltype"), ncores = nCores )
