...
cells <- normalizeCells(cells,transformation = "sqrt", method = c("99perc"), assayIn = "norm")
...
cells <- runFuseSOM(cells, assay = 'norm', numClusters = 20)
...
```{r}
plotGroupedHeatmap(cells, useMarkers, group = "clusters", exprs_values = "norm", center = TRUE, scale = TRUE, zlim = c(-2,2))
```
```{r}
...
```{r}
cells <- lisaClust::lisaClust(cells, k = 5, Rs = c(20,50,100), spatialCoords = c("m.cx", "m.cy"), cellType = "clusters", BPPARAM = BPPARAM)
