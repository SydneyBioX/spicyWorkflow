...
BPPARAM <- simpleSeg:::generateBPParam(5)
...
gc()
 ...
 images <- loadImages("Data/h5Files/", "h5", on_disk = TRUE)
 mcols(images) <- S4Vectors::DataFrame(imageID = names(images))
```
...
cells <- cytomapper::measureObjects(masks3, 
                                    images, 
                                    img_id = "imageID", 
                                    BPPARAM = simpleSeg:::generateBPParam(cores = 6))
...
cells <- normalizeCells(cells, transformation = "sqrt", method = c("99perc"), assayIn = "counts")
...
data <- data[labels!="cluster_6",]
labels <- labels[labels!="cluster_6"]
